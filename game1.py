...
print("Debugging2\n")
tuple_list = [tuple(row) for row in G.full_payoff_table().itertuples()]
count = 0
for i in range(18):
    for j in range(i+1,18):
        matrix = np.zeros((2,2))
        matrix[0][0] = tuple_list[i][1]
        matrix[0][1] = tuple_list[i][2]
        matrix[1][0] = tuple_list[j][1]
        matrix[1][1] = tuple_list[j][2]
        count+=1
        print("==============")
        print(matrix)
print(count)
